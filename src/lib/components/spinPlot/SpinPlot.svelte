<script lang="ts">
	...
	onMount(async () => {
		...
		width = container.offsetHeight / aspectRatio - margin.right - margin.left - boardHeight;
		if (maxWidth) maxWidth.set(width);
		height = container.offsetHeight - boardHeight;
		const maxRotate = 30;
...
<div class="viz-container" bind:this={container}>
	<svg bind:this={svg} width={width + margin.left + margin.right} {height}>
	...
	<SpinEsgBoard bind:board data={companyData} {year} {maxWidth} />
